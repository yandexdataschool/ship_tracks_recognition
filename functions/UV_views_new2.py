...
def points_crossing_line_xz(k, b, width, hits, intersecting_hits, n_min, clf, event, event2):
    """
    ...
    score, lr, track = turbo_fit(event.loc[crossing_points], event2, clf, view='Stereo')
    ...
    if n < n_min:
        ...
        for z in marks:
            ...
            for i in marks[z]:
                ...
                if (crossing_points == i).sum() != 0:
                    hits[z][i].used = True

        return 1, crossing_points, lin_regr

def loop_xz(event, tracks, linking_table, n_min, width, ind, clf, event2):
    """
                        ...
                        for end_z in (set(end_zs) & set(intersecting_hits)):
                            ...
                            for j in intersecting_hits[end_z]:
                                ...
                                if ((not hits[start_z][i].used) & (not hits[end_z][j].used)):
                                    ...
                                    indicator, crossing_points, lin_regr = \
                                        points_crossing_line_xz(new_k, new_b, width, hits, intersecting_hits, m, clf, event, event2)
